...
//var trafficURL = (window.location.href.split(':',1)[0] === 'file'?'http://cetrio.cyberleo.com.br':'')+'/traffic.php?id=';
var cameraURL = 'camera.php?id=';
var trafficURL = 'traffic.php?id=';
var sufixoURL = '.jpg';
    ...
    for (var j = 0; j < zonas.length; ++j) {
        ...
        for (var i = 0; i < cameras.length; ++i) {
            ...
            if (camera.zona == j) {
                ...
                tabBody += '<p id="caption-' + (1+i) + '" style="font-size:10pt;"></p>';
                //tabBody += '</p>';
            }
        }
        ...
        for (var j = 0; j < zonas.length; ++j) {
            for (var i = 0; i < cameras.length; ++i) {
                if (camera.zona == j) {
                    var trf = trafficURL + (1+i);
                    $('#caption-'+(1+i)).load(trf);
                }
            }
        }
    }
