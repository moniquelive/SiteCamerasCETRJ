    ...
    angular.module('camerasrj', ['ngRoute'])
      ...
      .config(['$routeProvider', function ($routeProvider) {
        $routeProvider
          ...
          .otherwise({ redirectTo: '/zona/AvBrasil' })
        ;
      }])

      .controller('ZonaCtrl', ['$scope', '$routeParams', 'CAMERAS', 'CAM_PREFIX', 'LIVE_PREFIX',
        function ($scope, $routeParams, cameras, camPrefix, livePrefix) {
          $scope.CAM_PREFIX = camPrefix;
          $scope.LIVE_PREFIX = livePrefix;
          this.cameras = cameras[$routeParams['zona'] || "AoVivo"];
        }])

      .factory('links', ['$rootScope', '$location', function ($rootScope, $location) {
        var coll = {};

        $rootScope.$on("$routeChangeSuccess", function (event, current, previous) {
          if (!_.size(coll)) return;
          if (previous && previous.params && previous.params.zona) {
            coll[previous.params.zona].selected = false;
          }
          if (current && current.params && current.params.zona) {
            coll[current.params.zona].selected = true;
          }
        });
        ...
        return {
          register: function (link) {
            ...
            link.selected = url.length > 1 && !!link.href.match(url);

            coll[link.title] = link;
          }
        };
      }])
