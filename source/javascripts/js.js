  ...
  window.init = function () {
    angular.module('camerasrj', ['ngRoute'])
      ...
      .run(["$rootScope", function ($rootScope) {
        $rootScope.CAM_PREFIX = CAM_PREFIX;
        $rootScope.LIVE_PREFIX = LIVE_PREFIX;
        $rootScope.tabs = _.map(_.pluck(CAMERAS.zonas, 'title'),
          function (e) {
      ...
      .config(['$routeProvider', function ($routeProvider) {
        $routeProvider
          ...
          .otherwise({
            redirectTo: '/zona/AvBrasil'
          })
        ;
      }])

      .controller('ZonaCtrl', ['$scope', '$routeParams', function ($scope, $routeParams) {
        var title = $routeParams['zona'] || "AoVivo";
        this.cameras = _.findWhere(CAMERAS.zonas, {title: title}).cameras;
      }])
